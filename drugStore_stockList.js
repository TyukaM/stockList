...
/* カテゴリを作成 */
drug_store_category_list.forEach( category => {
  ...
  document.querySelector("#drug_store_stocks").appendChild(div);
});
...
function getStocksData() {
  ...
  request.onsuccess = (event) => {
    ...
    .transaction([drugStore_db],"readonly")
...
const createStockList = (stock) => {
  ...
  item_change.addEventListener("click", () => {
    ...
    // カテゴリ
    for(let i=0; i<drug_store_category_list.length; i++){
      const category_opt = document.createElement("option");
      category_opt.value = drug_store_category_list[i].txt;  //value値
      category_opt.text = drug_store_category_list[i].txt;   //テキスト値
    ...
    item_delete.addEventListener("click", () => {
      ...
      request.onsuccess = (event) => {
        ...
        // トランザクションを開き、オブジェクトストアにアクセス
        const stocks_ObjectStore = db
        .transaction([drugStore_db],"readwrite")
        .objectStore(drugStore_db);
  ...
  dd.addEventListener("click", () => {
    if (dd.classList.contains("active")) {
      ...
      const all_dd = document.querySelectorAll("#drug_store_stocks dd");
      all_dd.forEach(item => {
