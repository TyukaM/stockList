...
/* カテゴリを作成 */
emergency_item_category_list.forEach( category => {
  ...
  document.querySelector("#emergency_item_stocks").appendChild(div);
});
...
function getStocksData() {
  ...
  request.onsuccess = (event) => {
    ...
    .transaction([emergencyItem_db],"readonly")
...
const createStockList = (stock) => {
  ...
  item_change.addEventListener("click", () => {
    ...
    // カテゴリ
    for(let i=0; i<emergency_item_category_list.length; i++){
      const category_opt = document.createElement("option");
      category_opt.value = emergency_item_category_list[i].txt;  //value値
      category_opt.text = emergency_item_category_list[i].txt;   //テキスト値
    ...
    item_delete.addEventListener("click", () => {
      ...
      request.onsuccess = (event) => {
        ...
        // トランザクションを開き、オブジェクトストアにアクセス
        const stocks_ObjectStore = db
        .transaction([emergencyItem_db],"readwrite")
        .objectStore(emergencyItem_db);
  ...
  dd.addEventListener("click", () => {
    if (dd.classList.contains("active")) {
      ...
      const all_dd = document.querySelectorAll("#emergency_item_stocks dd");
      all_dd.forEach(item => {
